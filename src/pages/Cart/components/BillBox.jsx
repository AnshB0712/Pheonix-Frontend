...
const BillBox = ({totalValue,items}) => {
  return (
    <Group spacing={0} style={{width:'100%',borderTop:'1px dashed grey',borderBottom:'1px dashed grey'}}>
        <Group position='apart' style={{width:'100%',padding:"10px 0",display:'grid',gridTemplateColumns:'1fr 1fr 1fr',placeItems:'center'}}>
            {items?.map(obj => (
                <>
                 <Text fz={12} color={'dimmed'} fw={600}>{`${obj.itemName}(${obj.qty}X)`}</Text>
